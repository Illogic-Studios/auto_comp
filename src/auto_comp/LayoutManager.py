...
class LayoutManager:
    ...
    def __backdrop_data(self, backdrop_longname):
        ...
        for i, bd_sn in enumerate(backdrop_shortnames):
            if bd_sn not in current_bd["backdrops"]:
                ...
                if len(parent_long_name) == 0:
                    parent_long_name = None
                long_name = ".".join(backdrop_shortnames[: i + 1])
                current_bd["backdrops"][bd_sn] = LayoutManager.__get_init_backdrop_data(
                    long_name, parent_long_name
                )
    ...
    #
    def add_top_level_backdrop_layout_relation(
        self,
        rel_backdrop_longname,
        backdrop_longname,
        position=POS_RIGHT,
    ...
    #
    def add_node_layout_relation(
        self, base_node, node_to_place, position=POS_RIGHT, mult_distance=1
    ):
        """
        Add node relation
        :param base_node
        ...
        def __compute_build_layout_backdrop(bd_data):
            ...
            if len(backdrops) == 0 and len(nodes) == 0:
                ...
                for node in nodes:
                    ...
                    if bd_x is None or bd_x > n_x:
                        bd_x = n_x
                    if bd_y is None or bd_y > n_y:
                        bd_y = n_y
                    if bd_x2 is None or bd_x2 < n_x2:
                        bd_x2 = n_x2
                    if bd_y2 is None or bd_y2 < n_y2:
                        bd_y2 = n_y2
                # CHILDREN BACKDROPS
                for child_bd_data in backdrops.values():
                    # Call recursively to compute the backdrop layout
                    data_child_bd_layout_backdrop = __compute_build_layout_backdrop(
                        child_bd_data
                    )
                    if data_child_bd_layout_backdrop is None:
                        continue
                    child_bd_x, child_bd_y, child_bd_w, child_bd_h = (
                        data_child_bd_layout_backdrop
                    )
                    child_bd_x2 = child_bd_x + child_bd_w
                    child_bd_y2 = child_bd_y + child_bd_h
                    if bd_x is None or bd_x > child_bd_x:
                        bd_x = child_bd_x
                    if bd_y is None or bd_y > child_bd_y:
                        bd_y = child_bd_y
                    if bd_x2 is None or bd_x2 < child_bd_x2:
                        bd_x2 = child_bd_x2
                    if bd_y2 is None or bd_y2 < child_bd_y2:
                        bd_y2 = child_bd_y2
                if displayed:
    ...
    def __compute_top_level_backdrops_relation(self):
        ...
        def __compute_backdrops_relation_aux(bd_longname, bd_lyt_data):
            ...
            base_bd_x, base_bd_y = (
                base_bd_layout_data["xpos"],
                base_bd_layout_data["ypos"],
            )
            base_bd_w, base_bd_h = (
                base_bd_layout_data["width"],
                base_bd_layout_data["height"],
            )
    ...
    def build_layout_backdrops(self):
        ...
        def __build_backdrop_aux(backdrop_name, backdrop_data, z_order):
            ...
            return nuke.nodes.BackdropNode(
                name=backdrop_name,
                xpos=int(layout_data["xpos"]),
                ypos=int(layout_data["ypos"]),
                bdwidth=layout_data["width"],
                bdheight=layout_data["height"],
                z_order=z_order,
                label=backdrop_name,
